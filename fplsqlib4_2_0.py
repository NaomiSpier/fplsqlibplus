...
__all__ = ['fplsqGUI', 'fplsqAB',  'fplsqA', 'fplsqB', 'get_t95']
...
class Data:
    ...
    def __init__(self, x, y, yerr=None, xerr=None):
        self.set(x, y, yerr, xerr)
    
    def set(self, x, y, yerr=None, xerr=None):
        self.x = x 
        self.y = y 
        self.yerr = yerr 
        self.xerr = xerr
    ...
    def get(self, xrange=[-np.inf, np.inf]):
        ...
        xerr = self.xerr[datamask] if self.xerr is not None else None
        return self.x[datamask], self.y[datamask], yerr, xerr
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, model, data, xlabel, ylabel):    
        ...
        if self.data.yerr is None and self.data.xerr is None:
    ...
    def fit(self):
        ...
        x, y, ye, xe = self.data.get(self.plotwidget.canvas.get_range())
        if weighted == 'none':
            ye = np.ones(len(y))
            
        elif ye is None:
            if weighted == 'relative':
                self.showdialog("Bro wat probeer je je hebt helemaal geen y-fout, maar oke ik stel hem wel op none imbeciel")
                weighted = 'none'
            ye = np.ones(len(y))
        ...
        with warnings.catch_warnings():
            ...
            try:
                fitpars, fitcov = curve_fit_wrapper(func, x, y, p0=p0, pF=pF, sigma=ye, dx=xe, weighted=weighted, jac=jac)
                ...
                for par, fitpar, err, stderr in zip(self.model.pars, fitpars, errors, stderrors):
                    par['value'] = fitpar
                    par['err'] = err
                    par['stderr'] = stderr
...
def curve_fit_wrapper(func, *pargs, p0=None, pF=None, jac=None, dx=None, weighted=False, sigma, **kwargs):
    ...
    if weighted=='relatief met x erbij':
        if dx is None:
            ...
            a = popt[0]
            dy = sigma
            while  abs(a-A) > max(rel_err * max(abs(a), abs(A)), 0.0):
                print()
                A = a
                sigma = np.sqrt(dy**2 + a**2*dx**2)
                popt, cov = curve_fit(fit_func, *pargs, p0=p0_fit, jac=fit_jac, sigma=sigma, absolute_sigma=weighted=='standard deviation', **kwargs) 
                a = popt[0]
...
def validate_input(func):
    ...
    def func_wrapper(xdata, ydata, *args, **kwargs):
        ...
        if 'xerr' in kwargs:
            if type(kwargs['xerr']) is not np.ndarray:
                raise Exception('data should have type numpy array')
            if len(kwargs['xerr']) != len(xdata):
...
@validate_input
def fplsqAB(xdata, ydata, yerr=None, xerr=None):
    """
    ...
    par, cov = curve_fit_wrapper(func, xdata, ydata, p0=[slope, offset], pF=[False, False], sigma=yerr, weighted=weighted)
    t95 = get_t95(len(xdata),2)
    return (par[0], par[1]), (np.sqrt(cov[0,0]), np.sqrt(cov[1,1])), (np.sqrt(cov[0,0])*t95, np.sqrt(cov[1,1])*t95)

@validate_input
def fplsqA(xdata, ydata, yerr=None, xerr=None, offset=0):
    """
    ...
    yerr -- numpy array, default:None   used for weighted fit with
                                        a relative weight defined 
                                        as 1/yerr**2  
    xerr -- numpy array, default:None   used for weighted fit with
                                        a relative weight defined 
                                        as 1/(yerr**2+a**2*xerr**2)
    ...
    return par[0], np.sqrt(cov[0,0]), np.sqrt(cov[0,0]) * t95

@validate_input
def fplsqB(xdata, ydata, slope, yerr=None, xerr=None):
    """
    ...
    example of use:
        ...
        #optionally define the errors in the ydata and xdata
        yerr = np.array([0.5,0.4,0.6,0.5,0.8])
        xerr = np.array([6,9,4,2,0])
    ...
    t95 = get_t95(len(xdata),1)
    return par[1], np.sqrt(cov[1,1]), np.sqrt(cov[1,1]),*t95   



@validate_input
def fplsqGUI(xdata, ydata, yerr=None, xerr=None, xlabel='x-values', ylabel='y_values'):   
    """
    ...
    example of use:
        ...
        #execute the function
        fplsqGUI(xdata, ydata, yerr=yerr, xerr=xerr, xlabel=xlabel, ylabel=ylabel)
    ...
    def func(x, a, b):
        ...
        """
        return a*x + b
    _curve_fit_GUI(func, xdata, ydata, yerr=yerr, xerr=xerr, xlabel=xlabel, ylabel=ylabel)  
    return None

@validate_input
def fplsqGUI_2order(xdata, ydata, yerr=None, xerr=None, xlabel='x-values', ylabel='y_values'):   
    """
...
def _curve_fit_GUI(func, xdata, ydata, yerr=None, xerr=None, xlabel='x-values', ylabel='y_values'):   
    """
    ...
    mydata = Data(xdata, ydata, yerr, xerr)
...
def get_t95(N, ddof):
    """
    Calculates the t95 value with scipy stats.

    Parameters
    ----------
    N : number of meassurements.
    ddof : the number of value calculated with the meassurements.
...
if __name__ == "__main__":
    ...
    fplsqGUI(x, y, xerr=dx, xlabel='Frequency [rad/s]', ylabel='Normalized Voltage U/U0')
